...
- (id)init
{
    if((self = [super init])) {
        ...
        yaml_emitter_set_output(&emitter, (yaml_write_handler_t*)CFDataAppendBytes, (__bridge void*)buffer);
...
- (void)emitItem:(id)item
{
    ...
    yaml_document_initialize(&document, NULL, NULL, NULL, !self.usesExplicitDelimiters, !self.usesExplicitDelimiters);
...
- (int)_writeItem:(id)item toDocument:(yaml_document_t *)doc
{
	...
	// Everything else is a scalar.
		...
		nodeID = yaml_document_add_scalar(doc,
                                          (yaml_char_t *)YAML_DEFAULT_SCALAR_TAG,
                                          (yaml_char_t*)[[item description] UTF8String],
                                          (int)[[item description] length],
